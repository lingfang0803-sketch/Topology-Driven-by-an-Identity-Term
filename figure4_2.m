...
index = find(real(E_exact) >= 0.6 & real(E_exact) <= 0.65 & ...
               imag(E_exact) >= -0.01 & imag(E_exact) <= 0.01);
N1=index(1);N2=index(2); %边模位置

psi1 = abs(V_exac(:, N1)).^2;   
psi2 = abs(V_exac(:, N2)).^2;   
Z = NaN(2*N, 2*N);  
Z(31,:) = psi1;   
Z(32,:) = psi2;
h(1)=mesh(1:2*N,1:2*N,Z);
