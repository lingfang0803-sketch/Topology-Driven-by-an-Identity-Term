...
xlabel('$\delta_2$','Interpreter','latex', 'FontSize', 16); ylabel('$|E|$ ','Interpreter','latex', 'FontSize', 16);
...
N = 30; 
tL =2; beta = pi/4; alpha=pi/3; delta1=1;
tR =1;    delta2=-0.8;
G=50;
...
syms Omega 
tic
Omega_roots  = sym(zeros(2*N, 3));
Theta_all =sym(zeros(2*N, 1));
Phi_all =sym(zeros(2*N, 1));
Psi_all =sym(zeros(2*N, 1));
for j=1:2*N
    E_single=E_exact(j);
    A =vpa(-(-E_single^2 + C)/D) ;
    B =vpa(B2*E_single^2+B1*E_single+B0) ;
    Gamma = vpa(Gamma2*E_single^2+Gamma1*E_single+Gamma0);
    CE=Omega^3-A*Omega^2+B*Omega-Gamma;
    Omega_roots = vpa(root(expand(CE),Omega)); 
    X= Omega_roots(1);
    Y= Omega_roots(2);
    Z= Omega_roots(3);
    [Theta, Psi, Phi] = genPoly(G,tL,tR,alpha, beta,delta1,delta2,E_single,X,Y,Z);
    Theta_all(j,:)=Theta;
    Psi_all(j,:)=Psi;
    Phi_all(j,:)=Phi;
end
...
z1=r*exp(1/2 *1i*(Theta_all+Phi_all-Psi_all));
z2=r*exp(1/2 *1i*(Theta_all-Phi_all+Psi_all));
z3=r*exp(1/2 *1i*(-Theta_all+Phi_all+Psi_all));
z4=r*exp(-1/2 *1i*(Theta_all+Phi_all+Psi_all));
...
for j = 1:2*N
   plot(1:2*N, abs(V_exac(:, j)).^2,'Color', colors{j},LineWidth=1);
end
xlabel('Site number');  ylabel('eigenvector number'); zlabel('Intensity');
xlim([1 2*N]);
xlabel("\sl n",FontSize=16); ylabel("|\psi|^2",FontSize=16);
